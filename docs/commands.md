...
| Command | What it does |
|---|---|
| `orion relay-serve` | Run the relay locally (ingest endpoint + read-only dashboard) on `127.0.0.1:8787`. Needs `ORION_RELAY_USER_PEPPER` in `.env` (pushes authenticate with contributor keys; see `docs/dashboard-auth.md`). Blocks until Ctrl-C. |

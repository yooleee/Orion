...
- **Inc 4 — sectioned dashboard rebuild (richer-client SPA)** — IN PROGRESS. A full Claude-design
  ...
  **Scheduling SHIPPED** (branch `e2-inc4-4a-scheduling`): a pure read-only cross-project derivation —
  ...
  green. **First production deployment IN FLIGHT** — the SPA cutover, deployed as a NEW Fly app
  `project-orion.fly.dev` (Fly has no in-place rename), retiring the old `orion-relay-horizon-c`;
  production-cutover accepted (the inert composer is the only regression, closed by comment writes). **Still
  ahead in 4a's band:** comment writes, the Showcase guest view, the mobile pass, and retiring `render.py`
  at parity. **Remaining sub-steps:** **4b**

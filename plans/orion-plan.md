...
| E2 | dashboard | Dashboard as a richer multi-signal, multi-project visibility/showcase surface (idea #5) — portfolio map + cross-project visibility + the to-do/milestone signal, a forward-looking layer, and a full **sectioned** redesign | 🛠️ **Inc 1–3 SHIPPED + DEPLOYED.** Inc 1 (portfolio overview), Inc 2 (live checklist signal, PR #47), Inc 2.5 (near-real-time push, PR #49), Inc 2.6 (status-aware `tracker` collector, PR #50), a consolidation slice (PRs #52–#54), and **Inc 3 (forward-looking layer ≡ E1, Units 0–5, deployed 2026-06-26)** all done. **Inc 4 — sectioned dashboard rebuild (richer-client SPA) — COMPLETE.** Recreating the ingested Claude-design (`design/`) as a **React/Vite SPA, single-host on the relay** (relay → read-only JSON API); server-rendered HTML retires at parity (kept working meanwhile). **4a BUILT (in review):** relay → read-only JSON API (`relay/api.py`: me/portfolio/project/report) + JSON auth + explicit `kind` (project/tracker) flag; the `web/` SPA (Vite/React/TS) with the 3-theme `data-theme` token system, shell+routing+login, **Projects home** (rows + tracker card + scope banner + empty), **Project page**, **Report detail**; relay serves the built SPA single-host (`--web-dir` + SPA CSP + path-traversal guard + SPA fallback; Dockerfile multi-stage). **Tracker page SHIPPED** (gap-8/KI-22 closed via a first-class producer `status` field; verified across all three themes). **Scheduling SHIPPED** (`GET /api/scheduling` — pure read-only cross-project deadline bucketing; verified across all three themes). **First production deployment SHIPPED** — SPA cutover to a NEW Fly app `project-orion.fly.dev` (Fly has no in-place rename; DB + secrets migrated, old `orion-relay-horizon-c` destroyed). **Comment writes SHIPPED** (`POST /api/reports/:id/comments` — cookie-authed JSON reusing the form route's CSRF/auth/scope/identity guards; eyes-on incl. XSS-inert). **Showcase guest view SHIPPED** (PR #65: opt-in + default-deny `GET /api/showcase`, summary cards only, 404 when disabled; relay `--showcase` flags). **Mobile pass SHIPPED** (PR #66: first `@media(max-width:768px)` rules — sidebar → bottom tab bar + mobile top bar; eyes-on all screens × 3 themes). **`render.py` RETIRED at parity — 4a band COMPLETE** (closes KI-23: deleted `relay/render.py` + the legacy form routes `GET/POST /login` and `POST /report/:id/comment`; SPA is the only front-end, relay API-only without `--web-dir`; JSON API + cookie comment write + auth/CSP + store/derive kept; backend 676 + web 33 green). **4b Disciplines & directions SHIPPED** — a doc-centric collector reads a project's own docs UNCHANGED and reframes their stated principles into cards via an opt-in, cache-gated Haiku step (observe-not-originate; the collector stamps the repo-relative `source`, never the model, so `observed · <doc>` is honest). Seam mirrors the checklist push: `disciplines-push` → `POST /disciplines` → `relay_project_disciplines` → `GET /api/disciplines` (`serialize_disciplines`: Global/project split, dedupe, scope-filter-first) → `Disciplines.tsx`; verified vs `desktop-06` × 3 themes; backend 714 + web 36 green. **4c Skills comb SHIPPED** (replaces the design's `desktop-07` Connections — mostly-independent projects made a literal connections graph thin, so it was reframed with the developer into a Capability-Comb **"partial living resume"**): a per-project collector LLM-extracts skills **grounded only in observed activity** (git languages + commit subjects + doc focus), the relay **merges them across the portfolio** and derives each tooth's depth (weight + breadth), `Skills.tsx` renders the comb + evidence cards anchoring each skill to its projects. Seam mirrors 4b: `skills-push` (a `skills = true` flag) → `POST /skills` → `relay_project_skills` → `GET /api/skills` (`serialize_skills`: merge + depth + scope-first) → `Skills.tsx`; observe-not-originate holds; eyes-on on genuine extracted data × 3 themes; backend 744 + web 43 green (supersedes `design/` §8 + `desktop-07`; KI-25). **4c REWORK CP1 SHIPPED** (the initial 4c was judged not-yet-reasonable on real data — KI-26): `skills-push` → **`skills-sync`**, a **global two-pass** extraction (pass 1 = one deduplicated, resume-grade canonical vocabulary across all projects on Sonnet; pass 2 = per-project attribution, **blind to the others** → existence-hiding stays structural), written via an **atomic batch** endpoint (`POST /skills-batch`: one transaction + prune + empty-clobber guard); depth boundaries re-tuned for now-accurate breadth; backend 763 green. **4c REWORK CP2 SHIPPED** — `Skills.tsx` + `skillsComb.ts` + `base.css` redesigned to the true comb-shaped-skills form (horizontal spine = breadth, teeth hanging DOWN, length = depth, "broken comb", category-labelled segments, evidence cards kept); web 7/7 + build green; eyes-on-verified on real seeded data across all 3 themes. **4c rework COMPLETE (KI-26 resolved).** **E2 Inc 4 COMPLETE.** Kickoff: [`docs/e2-inc4-dashboard-rebuild-kickoff.md`](../docs/e2-inc4-dashboard-rebuild-kickoff.md). API contract: [`docs/dashboard-api-contract.md`](../docs/dashboard-api-contract.md). Ladder below. |
...
| E4 | both | Surface-plural coordination across multiple projects / cross-project (the registry already holds many) | 🛠️ The **developer's own** cross-project view shipped as **E2 Inc 4c** — reframed from a Connections graph into the **Skills comb** (`GET /api/skills` merges each project's observed skills across the portfolio). A literal cross-project *relationship* graph is deferred (mostly-independent projects made it thin). Multi-party cross-project coordination stays 🔭 Long-range (C3-gated) |
| E5 | dashboard | The **read-only → read-write dashboard** inflection — the architectural watershed (write paths, auth, hosting-as-primary). The point to watch | 🛠️ **First concrete step SHIPPED (merged 2026-06-29): the supervisor-interaction loop** (E2 Inc 5) — an ongoing, attributable, per-project **two-way discussion** (supervisor ↔ developer) with Orion as connective tissue + memory, sequenced **before** a dogfooding pause so Orion reaches "usable for a stretch." Decided 2026-06-28: Orion is **medium + memory first** (carries/threads/remembers/surfaces observed state, **generates nothing** — observe-not-originate airtight; grounded-responder is a later rung); **per-project thread** (not per-report); **first-class identity now** (`author_id` + `role` supervisor\|developer\|orion, a new `supervisor` role on `relay_users`). Extends **C3** identity; builds on the existing comment/observe-not-originate machinery. Kickoff: [`docs/supervisor-interaction-loop-kickoff.md`](../docs/supervisor-interaction-loop-kickoff.md) (5-unit ladder: store+role → endpoints+contract → CLI loop → SPA panel → optional "open directions" derivation). **Backend + CLI + SPA BUILT & MERGED (PR #74, 2026-06-29):** Unit 1 (`relay_discussion_items` store + `supervisor` role); Unit 2 (`POST /api/discussions/:project/items` cookie write + `discussions[]` on the project read + contract; identity server-derived, append-only, existence-hiding); Unit 3 (the developer's terminal half — **3a** Bearer machine routes `GET/POST /api/discussions`, **3b** `orion discussions pull/reply` with a local watermark; reply lands as role=developer); Unit 4 (the **SPA discussion panel** — a per-project two-way thread on the project page with colour-coded supervisor/developer role badges, XSS-inert bodies, role-gated composer). 799 backend+CLI tests + 50 web tests green; the full loop verified end-to-end against a running relay (CLI: supervisor posts → developer pulls + replies; SPA: eyes-on across dark/sepia/light + a live post that persisted server-side). **The loop is feature-complete across backend + CLI + dashboard, MERGED (PR #74, 2026-06-29).** Unit 5 (optional read-only "open directions" derivation — supervisor messages awaiting a reply) is **deferred-additive**. **Comments↔Discussion consolidation (decided 2026-06-28, on review):** seeing it rendered showed the project page had **two near-identical conversation surfaces** (Discussion + Comments) doing one job — the discussion loop is effectively the matured, identity-first, two-way successor to C2 comments. Direction: **fold comments into the discussion model**, staged. **Stage 1 SHIPPED (in #74):** the project page now carries only the project-level **Discussion** thread; per-report **Comments** stay on the report-detail page (report-scoped feedback). **Stage 2 (planned, deliberate follow-on after some dogfooding):** unify the data model — optional `report_id` context on discussion items so "comment on report N" is a tagged discussion message, retire `report_comments`/the comment UI **at parity** (precedent: `render.py`/KI-23), migrate existing comment rows, fold the bot `POST /api/comments` path. Kickoff: [`docs/stage2-comments-discussion-consolidation-kickoff.md`](../docs/stage2-comments-discussion-consolidation-kickoff.md) (settled vs. open decisions + the A–F unit ladder). Tracked as **KI-28**. Rest of E5 (full read-write, hosting-as-primary) stays 🔭 aspirational. |
...
- **Inc 4 — sectioned dashboard rebuild (richer-client SPA)** — IN PROGRESS. A full Claude-design
  ...
  **Scheduling SHIPPED** (branch `e2-inc4-4a-scheduling`): a pure read-only cross-project derivation —
  ...
  guards; verified eyes-on incl. XSS-inert render). **Showcase guest view SHIPPED** (PR #65: opt-in +
  default-deny `GET /api/showcase` — summary cards only, 404 when disabled; relay `--showcase` /
  `--showcase-project NAME:"blurb"` flags; `serialize_me.showcase_enabled`; description = curated blurb else
  observed headline; status derived from completion). **Mobile pass SHIPPED** (PR #66: the first
  `@media(max-width:768px)` rules — sidebar → `BottomTabBar` + a "More" sheet + `MobileTopBar`, toggled by
  CSS not JS; `navState.ts` shares active-section between the two navs; two-column grids collapse to one;
  eyes-on every screen across all three themes). **`render.py` RETIRED at parity — 4a band COMPLETE**
  (closes KI-23: deleted `relay/render.py` + the legacy form routes `GET/POST /login` and
  `POST /report/:id/comment`; the SPA is the only front-end and the relay is API-only without `--web-dir`;
  the JSON API, cookie comment write, auth/cookie/CSP machinery, `GET /logout`, and store/derive all kept;
  `_headline` moved to `api.py`, the comment caps + display-tz default to `server.py`; backend 676 + web 33
  green). **Sub-steps (all SHIPPED):** **4b**
  Disciplines & directions (a doc-centric signal: a collector reading CLAUDE.md/decision docs + a section);
  **4c** the **Skills comb** — reframed from the planned Cross-project Connections graph (mostly-independent
  projects made a literal relationship graph thin) into a Capability-Comb "partial living resume": a
  per-project collector LLM-extracts skills grounded in observed activity, the relay merges them across the
  portfolio and derives each tooth's depth, `Skills.tsx` renders the comb + evidence cards. **E2 Inc 4 is
  COMPLETE. Kickoff:**

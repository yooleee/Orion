...
| C2    | Bidirectional replies — supervisors comment back (dashboard first; native Discord/Slack threads as a richer add-on); brings inbound validation + authorization                                 | 🎯 **First slice scoped (2026-06-19): dashboard comments on the deployed relay** — reuse view-secret auth + an optional self-entered name; dashboard-only (push-to-developer deferred); bots/native replies deferred. The live Fly relay IS the always-on host. Inbound security (auth · stored-XSS · CSRF · validation) is the slice's center of gravity. Scoped in [`docs/c2-kickoff.md`](../docs/c2-kickoff.md); build in a fresh session. |

...
- **Inc 4 — sectioned dashboard rebuild (richer-client SPA)** — IN PROGRESS. A full Claude-design
  ...
  **4a SHIPPED (in review, sub-checkpoints 4a.0–4a.5):** the JSON API seam ([`docs/dashboard-api-contract.md`](../docs/dashboard-api-contract.md)
  ...
  Dockerfile). Verified end-to-end against live data across all three themes; backend + Vitest green. **Still
  ahead in 4a's band:** the full **Tracker page**, **Scheduling** (cross-project deadline aggregation), the
  Showcase guest view, the mobile pass, and retiring `render.py` at parity. **Remaining sub-steps:** **4b**

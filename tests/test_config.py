...
# --- skills flag (E2 Inc 4 slice 4c — the "skills comb") --------------------------


def test_skills_defaults_to_false(tmp_path):
    """A project without `skills` defaults to skills=False (opt-in).

    Why this matters: the comb is opt-in, so every project that predates 4c — and any
    project that does not set the flag — must surface no skills, unchanged.
    """
    path = _write(
        tmp_path,
        """
    ...
    assert get_project(load_config(path), "demo").skills is False


def test_skills_true_parses_without_a_companion_collector(tmp_path):
    """`skills = true` parses on a git-only project — no extra collector required.

    Why this matters: unlike `checklist`, skills are derived from signals the project
    already exposes (git languages + commits), so the flag stands on its own with no
    companion file/collector to declare.
    """
    path = _write(
        tmp_path,
        """
    ...
    assert get_project(load_config(path), "demo").skills is True


def test_skills_invalid_type_raises(tmp_path):
    """A non-boolean `skills` is a clear ConfigError, not a silently-truthy value.

    Why this matters: `skills = 1` / `"yes"` would otherwise be coerced to truthy; the
    isinstance(bool) check catches the typo at load with a fixable message.
    """
    path = _write(
        tmp_path,
        """
    ...
    with pytest.raises(ConfigError, match="invalid skills"):
        load_config(path)
